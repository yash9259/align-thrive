...
const CreatorCompanyCommunity = () => {
  ...
  const loadDirectory = async (id: string) => {
    const directory = await fetchCreatorCommunityDirectory(id, "brand");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const message = error instanceof Error ? error.message : "Unable to load company community.";
  ...
  const handleSendRequest = async (targetId: string) => {
    ...
    try {
      const result = await sendCreatorCommunityRequest(creatorId, targetId, "Let's connect in the company community.");
      await loadDirectory(creatorId);
      toast({
        title: result.status === "accepted" ? "Connection accepted" : "Request sent",
        description: result.status === "accepted" ? "You can message this company now." : "Waiting for company acceptance.",
      });
    } catch (error) {
  ...
  const handleRespond = async (requestId: string, action: "accepted" | "declined") => {
    ...
    try {
      ...
      toast({
        ...
        description: action === "accepted" ? "You can now message this company." : "Request was declined.",
      });
    } catch (error) {
  ...
  return (
    <DashboardLayout sidebar={<CreatorSidebar />} title="Company Community" userInitials={userInitials}>
      <div className="max-w-5xl space-y-5">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {!isLoading && !displayedMembers.length && <p className="text-sm text-muted-foreground">No companies found in this tab.</p>}
...
export default CreatorCompanyCommunity;
